...
struct Camera { Vector3 location; Vector3 rotation; float fov; };

struct FNRot {
	double a; char pad_0008[24];
	double b; char pad_0028[424];
	double c;
};
...
inline Camera get_view_point() {
	Camera view_point{};
	uintptr_t location_pointer = read<uintptr_t>(cache::uworld + CameraLocation);
	uintptr_t rotation_pointer = read<uintptr_t>(cache::uworld + CameraRotation);
	FNRot fnrot{};
	...
	view_point.fov = read<float>(cache::player_controller + CAMERAFOV) * 90.0f;
	return view_point;
}
...
inline bool is_visible(uintptr_t mesh) {
	auto Seconds = read<double>(cache::uworld + CameraRotation + 0x10);
