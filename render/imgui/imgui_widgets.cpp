...
bool ImGui::Checkbox(const char* label, bool* v, bool settings, IDirect3DTexture9* icon)
{
    ...
    const ImVec2 item_size(420 - anim.third, 50);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(item_size.x, item_size.y));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (is_visible)
    {
        ...
        if (settings && anim.third > 1) {
            ImColor bg_col2(12, 12, 12, int(255 * (anim.third / 100)));
            ImColor text_col2(255, 255, 255, int(255 * (anim.third / 100)));

            ImVec2 min = ImVec2(check_bb.Min.x + 330, check_bb.Min.y);
            ImVec2 max = ImVec2(check_bb.Min.x + 420, check_bb.Min.y + 50);
            draw_list->AddRectFilled(min, max, bg_col2, 5);

            ImVec2 min2 = ImVec2(check_bb.Min.x + 365, check_bb.Min.y + 15);
            ImVec2 max2 = ImVec2(check_bb.Min.x + 385, check_bb.Min.y + 35);
            draw_list->AddImage(icon, min2, max2, ImVec2(0, 0), ImVec2(1, 1), text_col2);

            if (mouse_pos.x > check_bb.Min.x + 330 &&
                mouse_pos.y > check_bb.Min.y &&
                mouse_pos.x < check_bb.Min.x + 420 &&
                mouse_pos.y < check_bb.Min.y + 50 && IsWindowFocused()) {
                if (IsMouseClicked(0)) {
                    OpenPopup(label);
                }
            }
        ...
        draw_list->AddRectFilled(check_bb.Min, check_bb.Max, bg_col, 5);

        {
            ImVec2 min = ImVec2(check_bb.Min.x + 380 - anim.third, check_bb.Min.y + 15);
            ImVec2 max = ImVec2(check_bb.Min.x + 400 - anim.third, check_bb.Min.y + 35);
            draw_list->AddRectFilled(min, max, box_col, 5);
        }
        {
            ImVec2 min = ImVec2(check_bb.Min.x + 385 - anim.third, check_bb.Min.y + 20);
            RenderCheckMark(draw_list, min, mark_col, 10);
        }
        
        const ImVec2 label_pos = ImVec2(check_bb.Min.x + 20, check_bb.Min.y + 25 - label_size.y / 2);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(420, 50));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
